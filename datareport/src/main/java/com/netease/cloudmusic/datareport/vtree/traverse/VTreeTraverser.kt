...
object VTreeTraverser: IVTreeTraverser {
    ...
    override fun buildViewTree(view: View, listDialogView: List<View>?, callback: IVTreeTraverseCallback): VTreeMap? {
        ...
        val virtualNode = VTreeNode(null, isVisible = false, virtual = false)
        ...
        if (tempRootPage == null) {
            ...
            performLogicView(view, nodeMap, callback, listDialogView)
        }
     ...
     * 处理逻辑挂载的view
     */
    private fun performLogicView(view: View, nodeMap: WeakHashMap<View, VTreeNode>, callback: IVTreeTraverseCallback, listDialogView: List<View>?) {
    ...
    private fun performInternal(nodeView: View, parentNode: VTreeNode, layer: Int, nodeMap: WeakHashMap<View, VTreeNode>, callback: IVTreeTraverseCallback, dataEntity: DataEntity?, isLogic: Boolean) {
        ...
        val entry = createVTreeNode(dataEntity?.elementId ?: dataEntity?.pageId, nodeView, tempRect, dataEntity, actualTempRect, parentNode, nodeMap, true)
        val listRef = dataEntity?.innerParams?.get(InnerKey.LOGIC_CHILDREN) as? List<WeakReference<View?>>? //逻辑挂载
    ...
    private fun performInternalPartial(nodeView: View, parentNode: VTreeNode, layer: Int, nodeMap: WeakHashMap<View, VTreeNode>, callback: IVTreeTraverseCallback, dataEntity: DataEntity?, isLogic: Boolean) {
        ...
        val entry = createVTreeNode(dataEntity?.elementId ?: dataEntity?.pageId, nodeView, tempRect, dataEntity, actualTempRect, parentNode, nodeMap, false)
     ...
     * 构建虚拟树节点
     */
    private fun createVTreeNode(id: String?, nodeView: View, tempRect: Rect, dataEntity: DataEntity?, actualTempRect: Rect, parentNode: VTreeNode,
                                nodeMap: WeakHashMap<View, VTreeNode>, appendPre: Boolean
    ) = if (id == null) null else VTreeNode(
        nodeView,
        isVisible = true,
        ...
        val viewParamNode = this.getInnerParam(InnerKey.VIEW_VIRTUAL_PARENT_NODE) //获取虚拟父节点
        if (viewParamNode is VTreeNode) { //虚拟父节点的处理，判断节点是否需要挂载到虚拟节点上去
            ...
            if (parentNode.childrenList.contains(virtualNode)) { //当前层级已经有一个对应的虚拟节点了
                val virtualParent =
                    parentNode.childrenList[parentNode.childrenList.indexOf(virtualNode)]
