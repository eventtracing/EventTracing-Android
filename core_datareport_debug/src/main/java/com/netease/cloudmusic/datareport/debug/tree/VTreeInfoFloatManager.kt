...
import com.netease.cloudmusic.datareport.automation.AutomationManager
...
class VTreeInfoFloatManager(target: ViewGroup) {
    ...
    private fun findAutoView(view: View, x: Int, y: Int): View? {
        ...
        if (AutomationManager.getSpm(view) != null && (x > off[0] && y > off[1] && x < off[0] + view.width && y < off[1] + view.height)) {
            return view
        }
        return null
    }
