...
object ReferManager : DefaultEventListener() {
    ...
    private fun updateHsRefer(hsRefer: String) {
        ReportUtils.getContext()?.let {
            SPUtils.put(it, HS_REFER_KEY, hsRefer)
        }
    }
    ...
    private val subPagePsReferMap = WeakHashMap<Activity, MutableMap<Int, String>>()

    private var subPageRefer: FinalData? = null
     ...
     */
    private fun isPageViewContainsRefer(node: VTreeNode): Boolean{
    ...
    fun onPageView(node: VTreeNode, pageViewData: FinalData, isRoot: Boolean) {
        if (isPageViewContainsRefer(node)) {
            return
        }
        ...
        if (isRoot) {
            ...
            if (generateReferEnable(node)) {
                subPageRefer?.reset()
                subPageRefer = FinalData().apply { putAll(pageViewData.eventParams) }
            }
        }
    ...
    private fun generateReferEnable(vTreeNode: VTreeNode): Boolean {
        return vTreeNode.getInnerParam(InnerKey.VIEW_GENERATE_REFER_ENABLE) == true
    }
     ...
     */
    fun onPageViewFix(oid: String, pageViewData: FinalData, isRoot: Boolean) {
        if (isRoot) {
    ...
    fun onSubPageExposure(vTreeNode: VTreeNode, userEvent: Boolean, userSubPage: Boolean) {
        findAttachedActivity(vTreeNode.getNode())?.let {
            var map = subPagePsReferMap[it]
            if (map == null) {
                map = mutableMapOf()
                subPagePsReferMap[it] = map
            }
            if (!map.containsKey(vTreeNode.hashCode())) {
                getSubPgRefer(vTreeNode.getOid(), userEvent, userSubPage)?.let { refer->
                    map[vTreeNode.hashCode()] = refer
                }
            }
        }
    }
    ...
    fun onRootViewExposure(vTreeNode: VTreeNode) {
        findAttachedActivity(vTreeNode.getNode())?.let {
            ...
            if (!isPageViewContainsRefer(vTreeNode)) {
    ...
    private fun getReferFinalDataFromList(vararg dataList: FinalData?): Pair<FinalData?, Boolean> {
        var targetData: FinalData? = null
        dataList.forEach { data ->
            if (data != null) {
                if (getFinalDataUploadTime(data) > getFinalDataUploadTime(targetData)) {
                    targetData = data
                }
            }
        }
        val isUndefine = targetData?.let {
            ((it.eventParams[UPLOAD_TIME] as? Long?) ?: 0) < getLastUndefineTime()
        } ?: false

        return Pair(targetData, isUndefine)
    }

    private fun getFinalDataUploadTime(data: FinalData?): Long {
        return (data?.eventParams?.get(UPLOAD_TIME) as? Long?) ?: 0
    }
    ...
    fun getSubPgRefer(oid: String, userEvent: Boolean, userSubPage: Boolean): String? {
        val pair = if (userEvent && userSubPage) {
            getReferFinalDataFromList(referStorage.getLastEventRefer(oid), referStorage.getLastPageView(), subPageRefer)
        } else if (userEvent) {
            getReferFinalData(oid)
        } else if (userSubPage) {
            getReferFinalDataFromList(referStorage.getLastPageView(), subPageRefer)
        } else {
            return null
        }
        return getReferStr(pair.first, null,  addUndefine = pair.second)
    }
    /**
     * 获取HsRefer
     * 埋点子线程
    ...
    fun getHsRefer(): String {
        ReportUtils.getContext()?.let {
            return SPUtils.get(it, HS_REFER_KEY, "")
        }
        return ""
    }
        ...
        if (finalData != null) {
            ...
            if (eventId == EventKey.APP_IN) {
                ...
                val pgStep = if (pageList != null && pageList.length() > 0) {
                    ...
                    if (pageList[pageList.length() - 1] is JSONObject) {
                        map = pageList[pageList.length() - 1] as JSONObject
                    } else if (pageList[pageList.length() - 1] is LinkedHashMap<*, *>) {
                        map = JSONObject(pageList[pageList.length() - 1] as LinkedHashMap<*, *>)
                    }

                    if (map != null && map.has(PAGE_STEP_KEY)) {
    ...
    private fun isStartWithSpm(spm: String, list: JSONArray?): Boolean {
        if (list != null && list.length() > 0) {
            ...
            var map: JSONObject? = null
            if (list[0] is JSONObject) {
                map = list[0] as JSONObject
            } else if (list[0] is LinkedHashMap<*, *>){
                map = JSONObject(list[0] as LinkedHashMap<*, *>)
            }

            if (map != null && map.has(OID_KEY) && spm.startsWith(map[OID_KEY] as String)) {
                return true
            }
        }
        return false
    ...
    //埋点子线程
    fun getSubPsRefer(vTreeNode: VTreeNode): String {
        findAttachedActivity(vTreeNode.getNode())?.let {
            return subPagePsReferMap[it]?.get(vTreeNode.hashCode()) ?: ""
        }
        return ""
    }
