...
public class StorageDurationTimer implements IDurationTimer {
    ...
    private final ProcessPreferences sp;
    ...
    public static final String APP_HEART_TIME = "app_heard_time";//心跳时间记录在sp里面跨进程
    ...
    public void startTimer() {
        reset();
        mBeatTimerKey = TimerTaskManager.getInstance().addTimerTask(this::updateDuration, TIMER_INTERVAL, TIMER_INTERVAL);
    }
    ...
    private void updateAppHeartTime() {
        sp.edit().putLong(APP_HEART_TIME, mDuration).apply();
    }
    /**
     * 心跳或appout打点。
     */
