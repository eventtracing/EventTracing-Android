...
public class FinalDataTarget {
    ...
    public static void handle(@Nullable final FinalData finalData) {
        innerHandler(finalData);
    }

    public static void handle(String oid, FinalData finalData, boolean isRoot) {
        if (finalData == null) {
            return;
        }
        ...
        ThreadUtils.runOnUiThread(() -> {
            ...
            addRealtimeExternalParams(publicParams);
            ...
            EventDispatch.INSTANCE.postRunnable(new Runnable() {
                @Override
                public void run() {
                    ReferManager.INSTANCE.onPageViewFix(oid, finalData, isRoot);
    ...
    private static void innerHandler(@Nullable final FinalData finalData) {
        if (finalData == null) {
            return;
        }
        String event = (String) finalData.eventParams.get(INNER_EVENT_CODE);
