...
fun findActivityByAny(obj: Any?): Activity? {
    if (obj is View) {
        return findAttachedActivity(obj)
    }
    if (obj is Dialog) {
        return DialogListUtil.getDialogActivity(obj)
    }
    if (obj is Activity) {
        return obj
    }
...
/**
 * 虚拟节点的childList的最后一个就是
 */
fun getRootByVTree(treeMap: VTreeMap?): VTreeNode? {
    return treeMap?.vTreeNode?.childrenList?.last()
}
/**
 * 获取根节点
 */
